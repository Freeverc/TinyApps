#include <graphics.h>
#include <conio.h>
#include <stdio.h>
#include <iostream>
#include <string.h>
#include <locale.h> 
#include <wchar.h>

#define _USE_MATH_DEFINES
#include <math.h>


void jiemian()
{
	// 创建绘图窗口
	initgraph(1000, 600);

	// 画色背景
	setfillcolor(RGB(0, 255, 128));
	solidrectangle(0, 0, 999, 599);

	//添加功能框
	setfillcolor(WHITE);//添加功能框颜色
	setlinecolor(BLACK);
	wchar_t gongNeng[9][4] = { L"画笔",L"直线",L"矩形",L"圆",L"椭圆",L"圆弧",L"扇形", L"清除" ,L"填充"};
	for (int i = 0; i < 9; i++)
	{
		fillrectangle(20, 40 * i + 20, 60, 40 * i + 40);//设置功能框大小
		setbkcolor(WHITE);//设置文字背景颜色
		settextcolor(BLUE);
		outtextxy(23, 40 * i + 23, gongNeng[i]);
	}

	//是否填充
	setlinecolor(BLACK);
	setfillcolor(WHITE);
	setbkcolor(WHITE);
	settextcolor(BLUE);
	settextstyle(14, 0, _T("宋体"));
	fillrectangle(80, 20, 140, 40);
	outtextxy(83, 23, L"空心图形");
	fillrectangle(160, 20, 220, 40);
	outtextxy(163, 23, L"填充图形");

	//添加颜色框
	setlinecolor(BLACK);
	COLORREF yanse[16] = { BLACK, BLUE, GREEN, CYAN, RED, MAGENTA, BROWN, LIGHTGRAY, DARKGRAY, 
		LIGHTBLUE, LIGHTGREEN, LIGHTCYAN, LIGHTRED, LIGHTMAGENTA, YELLOW,  WHITE };
	for (int i = 0; i < 16; i++)
	{
		setfillcolor(yanse[i]);
		fillrectangle(240 + 40 * i, 20, 260 + 40 * i, 40);//颜色框的大小
	}

	//画图窗口
	setfillcolor(WHITE);
	solidrectangle(80, 60, 980, 580);
	HRGN rgn = CreateRectRgn(80, 60, 980, 580);
	// 将该矩形区域设置为裁剪区
	setcliprgn(rgn);
}

int gaibianGongneng(int x, int y)//在功能框处改变功能
{
	if ((x < 20) || (x > 60) || (y < 20) || ((y - 20) % 40 > 20))
		return 0;
	else
		return (y - 20) / 40 + 1;
}

int gaibianYanse(int x, int y)//在颜色框上改变颜色
{
	if ((x < 240) || ((x - 240) % 40 > 20) || (y < 20) || (y > 40))
		return 0;
	else
		return (x - 240) / 40 +1;
}

int gaibianTianchong(int x, int y)//在填充框上改变填充
{
	if ((x < 80) || ((x - 80) % 80 > 60) || (y < 20) || (y > 40))
		return 0;
	else
		return (x - 80) / 80 + 1;
}

//获取角度
double getAngle(double x, double y)
{
	double angle = 1;
	if ((x > 0) && (y > 0))
	{
		angle = -atan(y / x);
	}
	else if ((x > 0) && (y < 0))
	{
		angle = -atan(y / x);
	}
	else if ((x < 0) && (y > 0))
	{
		angle = M_PI - atan(y / x);
	}
	else if ((x < 0) && (y < 0))
	{
		angle = M_PI - atan(y / x);
	}
	return angle;

}

int main()
{
	SetWorkingImage(NULL);
	jiemian();
	IMAGE zhenHuabu(1000, 600);
	IMAGE jiaHuabu(1000, 600);
	getimage(&zhenHuabu, 0, 0, 1000, 600);
	getimage(&jiaHuabu, 0, 0, 1000, 600);
	putimage(0, 0, &zhenHuabu);
	
	COLORREF yanse[16] = { BLACK, BLUE, GREEN, CYAN, RED, MAGENTA, BROWN, LIGHTGRAY, DARKGRAY,
	LIGHTBLUE, LIGHTGREEN, LIGHTCYAN, LIGHTRED, LIGHTMAGENTA, YELLOW,  WHITE };
	COLORREF dangqianYanse = yanse[0];
	setlinecolor(dangqianYanse);
	setfillcolor(dangqianYanse);
	int dangqianGongneng = 0;

	// 主循环
	MOUSEMSG m;
	int xInit, yInit;
	int xGangcai, yGangcai, rGangcai; 
	int xYuanxin = 360, yYuanxin = 270;
	int buttonDownFlag = 0;
	int tianchongFlag = 0;
	double a(0), b(0), x1, x2, y1, y2;
	double startAngle = 0, endAngle = 0;
	int Flag;
	int g;


	while (1)
	{
		m = GetMouseMsg();
		if (m.uMsg == WM_LBUTTONDOWN)
		{
			buttonDownFlag = 1;

			//改变功能
			g = gaibianGongneng(m.x, m.y);
			if (g != 0)
			{
				dangqianGongneng = g;
			}
			//改变填充
			g = gaibianTianchong(m.x, m.y);
			if (g != 0)
			{
				if (g == 1)
				{
					tianchongFlag = 0;
				}
				else if (g == 2)
				{
					tianchongFlag = 1;
				}
			}
			//改变颜色
			g = gaibianYanse(m.x, m.y);
			if (g != 0)
			{
				dangqianYanse = yanse[g - 1]; 
				setlinecolor(dangqianYanse);
				setfillcolor(dangqianYanse);
			}

			// 画图
			if ((m.x > 80) && (m.y > 60))
			{
				xInit = m.x;
				yInit = m.y;
				xGangcai = xInit;
				yGangcai = yInit;
				rGangcai = 0;			
				SetWorkingImage(NULL);
				getimage(&zhenHuabu, 0, 0, 1000, 600);
				getimage(&jiaHuabu, 0, 0, 1000, 600);
				SetWorkingImage(&jiaHuabu);
			}
		}
		
		else if (m.uMsg == WM_MOUSEMOVE)
		{
			if (((m.x > 80) && (m.y > 60)) && (buttonDownFlag == 1))
			{
				SetWorkingImage(&jiaHuabu);
				switch (dangqianGongneng)
				{
				case 1:
				{
					setlinecolor(dangqianYanse);
					line(xGangcai, yGangcai , m.x, m.y);
					break;
				}
				case 2:
				{
					setlinecolor(WHITE);
					line(xInit, yInit, xGangcai, yGangcai);
					setlinecolor(dangqianYanse);
					line(xInit, yInit, m.x, m.y);
					break;
				}
				case 3:
				{
					if (tianchongFlag == 1)
					{
						setfillcolor(WHITE);
						solidrectangle(xInit, yInit, xGangcai, yGangcai);
						setfillcolor(dangqianYanse);
						solidrectangle(xInit, yInit, m.x, m.y);
					}
					else
					{
						setlinecolor(WHITE);
						rectangle(xInit, yInit, xGangcai,yGangcai);
						setlinecolor(dangqianYanse);
						rectangle(xInit, yInit, m.x, m.y);
					}
					break;
				}
				case 4:
				{
					if (tianchongFlag)
					{
						setfillcolor(WHITE);
						solidcircle(xInit, yInit, rGangcai);
						setfillcolor(dangqianYanse);
						rGangcai = sqrt((m.x - xInit) * (m.x - xInit) + (m.y - yInit) * (m.y - yInit));
						solidcircle(xInit, yInit, rGangcai);
					}
					else
					{
						setlinecolor(WHITE);
						circle(xInit, yInit, rGangcai);
						setlinecolor(dangqianYanse);
						rGangcai = sqrt((m.x - xInit) * (m.x - xInit) + (m.y - yInit) * (m.y - yInit));
						circle(xInit, yInit, rGangcai);
					}		 
					break;
				}
				case 5:
				{
					if (tianchongFlag)
					{
						setfillcolor(WHITE);
						solidellipse(xInit, yInit, xGangcai, yGangcai);
						setfillcolor(dangqianYanse);
						solidellipse(xInit, yInit, m.x, m.y);
					}
					else
					{
						setlinecolor(WHITE); 
						ellipse(xInit, yInit, xGangcai, yGangcai);
						setlinecolor(dangqianYanse);
						ellipse(xInit, yInit, m.x, m.y);
					}
						
					break;
				}
				case 6:
				{
					setlinecolor(WHITE);
					arc(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
					setlinecolor(dangqianYanse);
					putpixel(xYuanxin, yYuanxin, BLACK);
					x1 = double(xInit - xYuanxin);
					y1 = double(yInit - yYuanxin);
					x2 = double(m.x - xYuanxin);
					y2 = double(m.y - yYuanxin);
					a = sqrt((x1 * x1 * y2 * y2 - x2 * x2 * y1 * y1) / (y2 * y2 - y1 * y1));
					b = sqrt((x2 * x2 * y1 * y1 - x1 * x1 * y2 * y2) / (x2 * x2 - x1 * x1));

					startAngle = getAngle(x1, y1);
					endAngle = getAngle(x2, y2);
					//startAngle = 1;
					//endAngle = 3;
					arc(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
					//arc(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, 0, 6);
					break;
				}
				case 7:
				{
					if (tianchongFlag == 1)
					{
						setfillcolor(WHITE);
						solidpie(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
						setfillcolor(dangqianYanse);
					}
					else
					{
						setlinecolor(WHITE);
						pie(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
						setlinecolor(dangqianYanse);
					}
					
					putpixel(xYuanxin, yYuanxin, BLACK);
					x1 = double(xInit - xYuanxin);
					y1 = double(yInit - yYuanxin);
					x2 = double(m.x - xYuanxin);
					y2 = double(m.y - yYuanxin);
					a = sqrt((x1 * x1 * y2 * y2 - x2 * x2 * y1 * y1) / (y2 * y2 - y1 * y1));
					b = sqrt((x2 * x2 * y1 * y1 - x1 * x1 * y2 * y2) / (x2 * x2 - x1 * x1));

					startAngle = getAngle(x1, y1);
					endAngle = getAngle(x2, y2);
					//startAngle = 1;
					//endAngle = 3;
					if (tianchongFlag == 1)
						solidpie(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
					else
						pie(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
					break;
				}
				default:
					break;
				}
				SetWorkingImage(NULL);
				putimage(0, 0, &jiaHuabu);
				xGangcai = m.x;
				yGangcai = m.y;
			}
		}
		
		else if (m.uMsg == WM_LBUTTONUP)
		{
			buttonDownFlag = 0;
			if (dangqianGongneng == 8)
			{
				SetWorkingImage(NULL);
				dangqianGongneng = 0;
				setfillcolor(WHITE);
				solidrectangle(80, 60, 980, 580);
				getimage(&zhenHuabu, 0, 0, 1000, 600);
				getimage(&jiaHuabu, 0, 0, 1000, 600);
				setlinecolor(dangqianYanse);
				setfillcolor(dangqianYanse);
				continue;
			}
			else
			{	
				if ((m.x > 80) && (m.y > 60))
				{	
					SetWorkingImage(&zhenHuabu);
					switch (dangqianGongneng)
					{
					case 1:
					{
						zhenHuabu = jiaHuabu;
						break;
					}
					case 2:
					{
						setlinecolor(dangqianYanse);
						line(xInit, yInit, m.x, m.y);
						break;
					}
					case 3:
					{
						if (tianchongFlag == 1)
						{
							setfillcolor(dangqianYanse);
							fillrectangle(xInit, yInit, m.x, m.y);
						}
						else
						{
							setlinecolor(dangqianYanse);
							rectangle(xInit, yInit, m.x, m.y);
						}
						break;
					}
					case 4:
					{
						if (tianchongFlag)
						{
							setfillcolor(dangqianYanse);
							fillcircle(xInit, yInit, sqrt((m.x - xInit) * (m.x - xInit) + (m.y - yInit) * (m.y - yInit)));
						}
						else
						{
							setlinecolor(dangqianYanse);
							circle(xInit, yInit, sqrt((m.x - xInit) * (m.x - xInit) + (m.y - yInit) * (m.y - yInit)));
						}
						break;
					}
					case 5:
					{
						if (tianchongFlag)
						{
							setfillcolor(dangqianYanse);
							fillellipse(xInit, yInit, m.x, m.y);
						}
						else
						{
							setlinecolor(dangqianYanse);
							ellipse(xInit, yInit, m.x, m.y);
						}
						break;
					}
					case 6:
					{
						putpixel(xYuanxin, yYuanxin, BLACK);
						double a, b, x1, x2, y1, y2;
						double startAngle, endAngle;
						x1 = double(xInit - xYuanxin);
						y1 = double(yInit - yYuanxin);
						x2 = double(m.x - xYuanxin);
						y2 = double(m.y - yYuanxin);
						a = sqrt((x1 * x1 * y2 * y2 - x2 * x2 * y1 * y1) / (y2 * y2 - y1 * y1));
						b = sqrt((x2 * x2 * y1 * y1 - x1 * x1 * y2 * y2) / (x2 * x2 - x1 * x1));
						
						startAngle = getAngle(x1, y1);
						endAngle = getAngle(x2, y2);
						//startAngle = 1;
						//endAngle = 3;
						arc(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
						//arc(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, 0, 6);
						break;
					}
					case 7:
					{
						putpixel(xYuanxin, yYuanxin, BLACK);
						double a, b, x1, x2, y1, y2;
						double startAngle, endAngle;
						x1 = double(xInit - xYuanxin);
						y1 = double(yInit - yYuanxin);
						x2 = double(m.x - xYuanxin);
						y2 = double(m.y - yYuanxin);
						a = sqrt((x1 * x1 * y2 * y2 - x2 * x2 * y1 * y1) / (y2 * y2 - y1 * y1));
						b = sqrt((x2 * x2 * y1 * y1 - x1 * x1 * y2 * y2) / (x2 * x2 - x1 * x1));

						startAngle = getAngle(x1, y1);
						endAngle = getAngle(x2, y2);
						//startAngle = 1;
						//endAngle = 3;
						if (tianchongFlag == 1)
						{
							setfillcolor(dangqianYanse);
							fillpie(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
						}
						else
						{
							setlinecolor(dangqianYanse);
							pie(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, startAngle, endAngle);
						}
						//arc(xYuanxin - a, yYuanxin - b, xYuanxin + a, yYuanxin + b, 0, 6);
						break;
					}
					case 9:
					{
						setfillcolor(dangqianYanse);
						floodfill(m.x, m.y, getpixel(m.x, m.y), 1);
					}
					default:
						break;
					}				
					SetWorkingImage(NULL);
					putimage(0, 0, &zhenHuabu);
				}

			}
		}

		else if (m.uMsg == WM_RBUTTONDOWN)
		{	
			SetWorkingImage(NULL);
			if (((m.x > 80) && (m.y > 60)) && ((dangqianGongneng == 6)||(dangqianGongneng == 7)))
			{
				xYuanxin = m.x;
				yYuanxin = m.y;
				putpixel(xYuanxin, yYuanxin, BLACK);
			}
		}
	}
	// 按任意键退出
	//_getch();
	closegraph();
}

